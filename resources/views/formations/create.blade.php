...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <h1 class="text-3xl font-bold text-gray-800 mb-8">Ajouter une formation</h1>
        ...
        <form method="POST" action="{{ route('formations.store') }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full border @error('description') border-red-500 @else border-gray-300 @enderror
                                 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="duree" name="duree" value="{{ old('duree') }}"
                           placeholder="ex: 20h"
